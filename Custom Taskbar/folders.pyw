...
def app_config():
    with open("Folders.json", "r") as config_file:
...
def main():
    ...
    TOTAL_HEIGHT = DOCK_ICON_SIZE + (PADDING * 93) + 2 # Padding bottom + border
    widget.hidden_pos = QPoint((SCREEN_WIDTH - widget.width()), SCREEN_HEIGHT - 1)
    widget.visible_pos = QPoint((SCREEN_WIDTH - widget.width()), SCREEN_HEIGHT - TOTAL_HEIGHT)
    widget.hide()
